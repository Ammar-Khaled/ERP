...
export class ReturnPurchaseItemService {
   ...
   */
    async create(returnPurchaseItemDto: CreateReturnPurchaseItemDto, transactionalEntityManager?: EntityManager): Promise<ReturnPurchaseItem> {
        ...
        const manager = transactionalEntityManager || this.returnPurchaseItemRepo.manager;
        ...
        const returnPurchaseItem = manager.create(ReturnPurchaseItem, returnPurchaseItemDto);
    ...
    async update(
        ...
        const returnPurchaseItem = await manager.findOne(ReturnPurchaseItem, { where: { id } });
        if (!returnPurchaseItem) {
            throw new NotFoundException(`Return purchase item with id ${id} not found`);
        }
