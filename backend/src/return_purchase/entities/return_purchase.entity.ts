...
export class ReturnPurchase {
  ...
  @ManyToOne(
    () => PurchaseRequest,
    (purchaseRequest) => purchaseRequest.returnPurchases,
    {
      nullable: false,
    },
  )
  ...
  @OneToMany(
    () => ReturnPurchaseItem,
    (returnPurchaseItems) => returnPurchaseItems.returnPurchase,
    {
      nullable: false,
      eager: true,
    },
  ...
  calculateTotalReturnedMoney() {
    this.totalReturnedMoney = this.returnPurchaseItems.reduce(
      (total: number, item: ReturnPurchaseItem) => total + item.returnedMoney,
      0,
    );
  }
