...
export class ReturnPurchaseItem {
  ...
  @ManyToOne(
    () => PurchaseItem,
    (purchaseItem) => purchaseItem.returnPurchaseItems,
    {
      nullable: false,
      eager: true,
    },
  ...
  calculateReturnedMoney() {
    if (this.purchaseItem) {
      this.returnedMoney =
        this.purchaseItem.purchaseEntity.unitPrice * this.numberOfReturned;
    }
  }
  ...
  @ManyToOne(
    () => ReturnPurchase,
    (returnPurchase) => returnPurchase.returnPurchaseItems,
  )
