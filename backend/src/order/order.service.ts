import { Injectable, Inject, NotFoundException, HttpException, HttpStatus, ForbiddenException } from '@nestjs/common';
...
export class OrderService {
  ...
  async create(createOrderDto: CreateOrderDto) {
    ...
    if(!branch){
      throw new NotFoundException(jsend.fail({message: 'There is NO branch with that id !!'}));
    }
    ...
    if(!user){
      throw new NotFoundException(jsend.fail({message: 'There is NO user with that id !!'}));
    }
    ...
    if(!client){
      throw new NotFoundException(jsend.fail({message: 'There is NO client with that id !!'}));
    }
    ...
    if(createOrderDto.coupon_id > 0){
      ...
      if(!coupon){
        throw new NotFoundException(jsend.fail({message: 'There is NO coupon with that id !!'}));
      }
    }
    ...
    try{
      ...
      throw new HttpException(
        jsend.error({message: 'An unexpected error occurred while trying to save the order. Please, try again later.',data: error}),
